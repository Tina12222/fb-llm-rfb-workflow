'''
调用大模型来从excel里面的abstract列里面提取研究的method
'''
...
Your task is to read the *Abstract* below and extract **one** most-representative *Method* from the allowed lists.                              

### Allowed Method labels： 
1. Computational Modeling & Multiscale Simulation
   eg. DFT, MD, CFD, multiphysics coupling, stack-level heat–mass transfer simulation
2. Machine Learning & Data-Driven Optimization
   eg. material/formulation performance prediction, Bayesian optimization, self-learning digital twin
3. Wet-Chemical Bulk Synthesis
   eg. solvothermal synthesis, co-precipitation, sol–gel processing, liquid-phase chemical reduction
4. Surface Functionalization
   eg. immersion coating, graft polymerization, molecular self-assembly, autocatalytic metal/metal-oxide deposition
5. Thermal Post-Processing & Radiation Processing
   eg. high-temperature annealing/carbonization, plasma modification, γ-/e-beam grafting, ion-beam etching
6. Additive Manufacturing & Advanced Forming
   eg. 3D-printed flow plates, templated electrode molding, freeze-casting, electrospinning
7. Operando & In-Situ Diagnostics
   eg. synchrotron CT, operando X-ray/neutron imaging, in-situ Raman/FTIR, online impedance spectroscopy
8. Techno-Economic & Lifecycle Analysis
   eg. TEA, LCA, cost–performance sensitivity, system-level energy efficiency and carbon-footprint assessment
9. Other
   eg. methods not covered by the categories above
                                                  

## Guideline：
1. **Output format** – reply with **one** JSON object and nothing else:
   **Example** → `{"method":"Computational Modeling & Multiscale Simulation"}`
2. **Label source** – pick each value **exclusively** from its own list
   *Method → Allowed Method labels*;
...
def extract_method(
    ...
    *,
    ...
    """输入 DataFrame，返回追加method 列的新 DataFrame。"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="批量抽取 redox‑flow battery 论文的Method 标签")
    parser.add_argument(
    "--infile",
    ...
    parser.add_argument(
        "--outfile",
        default=r"Reference\method_tag.xlsx",
        help="输出 xlsx 路径"
    )
    ...
    df_out = extract_method(df_in, model=args.model, concurrency=args.conc)
