...
def review_retrieval(
    ...
    1) 分别对 chunk 和 theme 通道使用 hybrid_similarity 检索，得到带权重的得分。
    ...
    try:
        ...
        logger.error("Error in theme hybrid_similarity: %s", e)
        theme_results = []

    logger.debug("theme_results: %s", theme_results)
