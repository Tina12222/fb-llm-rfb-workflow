...
from RAG.rag_core.config import DB_XLSX, THEME_BM25, THEME_FAISS
...
from RAG.rag_core.theme_retrieval import review_retrieval


def build_arg_parser() -> argparse.ArgumentParser:
    project_root = Path(__file__).resolve().parents[2]
    default_out = project_root / "RAG" / "retrieval" / "theme_only" / "retrieved_theme.xlsx"
    default_log = project_root / "RAG" / "retrieval" / "theme_only" / "theme_search.log"
    default_input = project_root / "RAG" / "accuracy_test.xlsx"

    parser = argparse.ArgumentParser(description="Run theme-only retrieval on a query Excel file.")
...
def setup_logger(log_file: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("theme_only")
...
def main() -> int:
    ...
    if args.dry_run:
        print("[dry-run] theme_only checks")
