...
from RAG.rag_core.chunk_retrieval import review_retrieval
from RAG.rag_core.config import CHUNK_BM25, CHUNK_FAISS, DB_XLSX
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    default_out = project_root / "RAG" / "retrieval" / "chunk_only" / "retrieved_chunk.xlsx"
    default_log = project_root / "RAG" / "retrieval" / "chunk_only" / "chunk_only_search.log"
    default_input = project_root / "RAG" / "accuracy_test.xlsx"

    parser = argparse.ArgumentParser(description="Run chunk-only retrieval on a query Excel file.")
    parser.add_argument("--input", type=Path, default=default_input)
    parser.add_argument("--output", type=Path, default=default_out)
    parser.add_argument("--log-file", type=Path, default=default_log)
    parser.add_argument("--db-xlsx", type=Path, default=Path(DB_XLSX))
...
def setup_logger(log_file: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("chunk_only")
...
def main() -> int:
    ...
    if args.dry_run:
        print("[dry-run] chunk_only checks")
    ...
    if not args.input.exists() or not args.db_xlsx.exists():
        logger.error("Missing required input or db file.")
        return 1
